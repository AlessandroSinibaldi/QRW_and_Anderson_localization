...
if(control == False):
    ...
    title = "Localizzazione degli autostati ($\mathit{W}$ = $\pi/6$)"
    plt.title(title)
    plt.xlabel("$\mathit{n}$")
...
if(control == True):
    for e in range(0, N):
        for j in range(0, N):
            ...
            for t in range (0, repetitions):
                ...
                for k in range(0, dim):
                    probabilities_tot[k, :] = np.add(probabilities[2*k,:], probabilities[2*k+1,:])
                    #probabilities_tot[k, :] = probabilities[2*k,:]                
    ...
    # Scrivere su file
    f = open(os.path.join(sys.path[0], "IPR_QRW.txt"), 'w')     # Quando si cambia dim, bisogna cambiare il nome del file su cui scrivere
    f.write(str(N))
    f.write("\n")
    for j in range(0, N):
