...
control = 0                                     # 0. calcolare lambda con diverso disordine
                                                # 1. generare distribuzione di Riccati (file di output 
                                                #    non caricato nel repository perché troppo pesante)
                                                # 2. xi disordine completo output 
                                                # 3. lambda(z) disordine debole output
...
if(control == 0):
    ...
    Lyapunov = np.zeros(n_energies)
    theta = m.pi/6
    r0 = 1
    N = 10000000

    def iterate(z, Lyapunov, theta, n, seed):
        rnd.seed(seed)
        r = r0
        for l in range(0, N):
            chi = rnd.uniform(-W/2, W/2)
            r = ((1/z)+z*cmt.exp(-im*chi)-(m.cos(theta/2)*cmt.exp(-im*chi))/r)/(m.cos(theta/2))
            Lyapunov[n] += m.log(abs(r))/N


    for j in range(0, len(z)):
        iterate(z[j], Lyapunov, theta, j, seed)
        seed+=1
    print(100*(np.max(Lyapunov)-np.min(Lyapunov))/(np.max(Lyapunov)+np.min(Lyapunov)))
...
if(control == 2):
    ...
    Lyapunov = np.zeros(n_thetas)
    r0 = 1
    N = 10000000

    def iterate(Lyapunov, theta, n, seed):
        rnd.seed(seed)
        r = r0
        for l in range(0, N):
            chi = rnd.uniform(-W/2, W/2)
            ...
            if(l >= int(N/10)):
                Lyapunov[n] += m.log(abs(r))/(N-N/10)

    for j in range(0, len(thetas)):
        iterate(Lyapunov, thetas[j], j, seed)
        seed+=1
    localization_lenght = [1/x for x in Lyapunov]

    f = open(os.path.join(sys.path[0], "Lyapunov_QRW_10^6.txt"), 'w')
...
if(control == 3):
    ...
    def iterate(Lyapunov, theta, a, z, b, seed):
        rnd.seed(seed)
        r = r0
        for l in range(0, N):
            ...
            if(l >= int(N/10)):
                Lyapunov[a][b] += m.log(abs(r))/(N-N/10)

    for j in range(0, n_thetas):
        for k in range(0, n_betas):
            iterate(Lyapunov, thetas[j], j, z[j][k], k, seed)
            seed+=1

    Lyapunov =  np.array([x/variance for x in Lyapunov])
    
    f = open(os.path.join(sys.path[0], "Lyapunov_vs_beta.txt"), 'w')
    f.write(str(n_betas))
    f.write("\n")
    f.write(str(n_thetas))
    f.write("\n")
    for j in range(0, n_thetas):
        ...
        for k in range(0, n_betas):
            ...
            f.write("\n")
            f.write ("{:.16f}".format(float(Lyapunov[j][k])))
            f.write("\n")